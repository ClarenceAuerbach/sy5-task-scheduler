...
First run the daemon process with:
...
```bash
./tadmor [-P PATH] [-l|-q|-w|-r TASKID|-x TASKID|-o TASKID|-e TASKID|-c|-s|-p|-i]
```
...
- `-c [-n] [-m MINUTES] [-H HOURS] [-d DAYSOFWEEK] CMD [ARG_1] ... [ARG_N]`  
  Creation of a simple task

- `-s [-n] [-m MINUTES] [-H HOURS] [-d DAYSOFWEEK] TASKID_1 ... TASKID_N`  
  Sequential combination of the tasks with identifiers `TASKID_1` to `TASKID_N`;  
  the command to be executed is `( CMD_1 ; ... ; CMD_N )`

- `-p [-n] [-m MINUTES] [-H HOURS] [-d DAYSOFWEEK] TASKID_1 ... TASKID_N`  
  Pipeline combination of the tasks with identifiers `TASKID_1` to `TASKID_N`;  
  the command to be executed is `( CMD_1 | ... | CMD_N )`

- `-i [-n] [-m MINUTES] [-H HOURS] [-d DAYSOFWEEK] TASKID_1 TASKID_2 [TASKID_3]`  
...
- `-w`  
  Display time until next task execution
...
3rd milestone:  
Redesigning architecture for cleaner and safer code  
